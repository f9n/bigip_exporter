...
func (c *VSCollector) Collect(ch chan<- prometheus.Metric) {
	...
	if err != nil {
		...
		logger.Warn("Failed to get statistics for virtual servers")
	} else {
		...
		logger.Debug("Successfully fetched statistics for virtual servers")
	}
	...
	logger.Debug("Getting virtual server statistics", "duration", elapsed)
}

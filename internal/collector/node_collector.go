...
func (c *NodeCollector) Collect(ch chan<- prometheus.Metric) {
	...
	if err != nil {
		...
		logger.Warn("Failed to get statistics for nodes", "error", err)
	} else {
		...
		logger.Debug("Successfully fetched statistics for nodes")
	}
	...
	logger.Debug("Getting node statistics", "duration", elapsed)
}
